...
class Conv_Model(nn.Module):
    def __init__(self, y_dim=3*32*32, decoder_dim=None, latent_dim=256, pretrained=False):
        ...
        if decoder_dim is None:
            self.decoder_dim = [latent_dim] + [1024, 2048]
        else:
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(3, 16, kernel_size=3, stride=1), # 16* 30 * 30
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2)) # 16 * 15 * 15
        
        self.layer2 = nn.Sequential(
            nn.Conv2d(16, 32, kernel_size=2, stride=1), # 32 * 14 * 14
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2, padding = 0)) # 32 * 7 * 7
        
        #self.drop_out = nn.Dropout()
        
        self.fc1 = nn.Linear(7 * 7 * 32, 2*latent_dim)
        ...
        if pretrained:
            self.load_weights('/scratch/yc3329/conv_encoder11_weights.pt', cuda=torch.cuda.is_available())
